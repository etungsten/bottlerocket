...
/// FriendlyVersion represents a valid version string that can be prefixed with or without 'v.
...
impl TryInto<semver::Version> for FriendlyVersion {
    ...
    fn try_into(self) -> Result<Version, Self::Error> {
        ...
        let version = if self.inner.starts_with('v') {
            &self.inner[1..]
        } else {
            &self.inner
        };
...
mod test_version {
    ...
    fn good_version_strings() {
        for ok in &[
            ...
            if *ok != "latest" {
                let _: Version = FriendlyVersion::try_into(FriendlyVersion {
                    inner: ok.to_string(),
                })
                .unwrap();
            }
